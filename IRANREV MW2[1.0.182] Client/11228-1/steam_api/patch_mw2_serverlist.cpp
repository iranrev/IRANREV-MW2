...
void UI_BuildServerDisplayList(int);
...
void ServerListInit()
{
	...
	// getserversResponse
	*(DWORD*)0x5AE114 = (DWORD)"takeserversResponse" ;//was "runmenuscript"; //was "getserversResponse"
...
void CL_GlobalServers_f( void ) {
	...
	NET_StringToAdr("server.iw4.ir", &to );//"192.168.1.2", "server.alteriw.net"
	//NET_StringToAdr(, &to );
...
void CL_ServersResponsePacket( msg_t *msg ) {
	...
	//Com_Printf(0, "CL_ServersResponsePacket\n");
			/*Com_Printf( 0, "server: %d ip: %d.%d.%d.%d:%d\n",numservers,
...
char* UI_LocalizeGameType(char* gameType ,char* fs_game)
{
	if (strlen(fs_game) != 0)
	{
		std::string str(fs_game);
		str.erase(0,5);

		char *mod = new char[str.length() + 1];
...
static int LAN_CompareServers( int source, int sortKey, int sortDir, int s1, int s2 ) {
	...
	switch( sortKey ) {
		...
		case SORT_CLIENTS:
			...
			break;
		case SORT_GAME:
			gt1 = UI_LocalizeGameType(server1->gameType , server1->mod);
			gt2 = UI_LocalizeGameType(server2->gameType , server2->mod);

			if (gt1 && gt2)
				res = _stricmp( UI_LocalizeGameType(server1->gameType , server1->mod), UI_LocalizeGameType(server2->gameType , server2->mod) );
			break;
		case SORT_HARDCORE:
...
{
	//Com_Printf(0, "[IRANREV] Entered into UI_StopServerRefresh\n");
...
static void UI_InsertServerIntoDisplayList(int num, int position) {
	//Com_Printf(0, "[IRANREV] Entered into UI_InsertServerIntoDisplayList\n");
	int i;
...
static void UI_RemoveServerFromDisplayList(int num) {
	//Com_Printf(0, "[IRANREV] Entered into UI_RemoveServerFromDisplayList\n");
	int i, j;
...
static void UI_BuildServerDisplayList(int force) {
	//Com_Printf(0, "[IRANREV] Entered into UI_BuildServerDisplayList\n");
...
{
	//Com_Printf(0, "[IRANREV] Entered into UI_DoServerRefresh\n");
	qboolean wait = false;
		...
		if (index >= 0 && index < uiInfo.serverStatus.numDisplayServers)
		{
			...
			switch (column) {
				...
				case SORT_CLIENTS : 
					...
					return clientBuff;
				case SORT_GAME : 
					return UI_LocalizeGameType(Info_ValueForKey(info, "gametype"),Info_ValueForKey(info, "fs_game"));
				case SORT_HARDCORE :
					if (hardcore & 1) {
						return "HC";// was "X"
					} else {
