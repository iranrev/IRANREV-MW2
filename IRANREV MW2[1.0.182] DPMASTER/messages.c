...
static void HandleInfoResponse (server_t* server, const char* msg)
{
	...
	// Validate server's API key
	value = SearchInfostring(msg, "apikey");
	if (value != NULL)
	{
		int e;
		TRY {
			...
			sprintf(apiURL, "http://api.alteriw.net/?valid=%s", value);
			webpage.contents = malloc(1);
			webpage.size = 1;

			handle = curl_easy_init();
			curl_easy_setopt(handle, CURLOPT_URL, apiURL);
			curl_easy_setopt(handle, CURLOPT_WRITEFUNCTION, write_data);
			curl_easy_setopt(handle, CURLOPT_WRITEDATA, &webpage);
			curl_easy_perform(handle);
			curl_easy_cleanup(handle);

			if (webpage.contents == "invalid")
			{
				Com_Printf (MSG_WARNING, "> WARNING: rejecting server %s (could not validate API key)\n",	peer_address);
				Sv_Remove(server);
				return;
			}
		} 
