...
//typedef char* (__cdecl * Cvar_ToString_t)(cvar_t* cvar);
//Cvar_ToString_t Cvar_ToString_f = (Cvar_ToString_t)0x484DD0;
...
char* Cvar_ToString(cvar_t* cvar)
{
	switch (cvar->type)
	{
		case 0:
			return (cvar->value.boolean) ? "1" : "0";
		case 1:
			return va("%g", cvar->value.value);
		case 5:
		case 6: // enum
			return va("%i", cvar->value.integer);
		case 7:
			return va("%s", cvar->value.string);
		default:
			return "";
	}
}
...
namespace aIW
{
	...
	{
		SayAll(gcnew String(aiw_sayName->value.string), message);
	}
	...
	{
		SayTo(clientNum, gcnew String(aiw_sayName->value.string), message);
	}
	...
	void AdminPluginBase::ExecuteCommand(String^ command, CommandRedirectDelegate^ redirect)
	{
		...
		Cmd_ExecuteString(0, 0, (char*)commandP.ToPointer());
		Com_EndRedirect();
	...
	{
		IntPtr nameP = Runtime::InteropServices::Marshal::StringToHGlobalAnsi(name);
		cvar_t* cvar = Cvar_FindVar((char*)nameP.ToPointer());
		Runtime::InteropServices::Marshal::FreeHGlobal(nameP);
		...
		return gcnew String(Cvar_ToString(cvar));
	}
